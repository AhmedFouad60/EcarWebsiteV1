...
@endsection
 @section('content')
    @component(layoutForm() , ['title' => trans('car.car') , 'model' => 'car' , 'action' => isset($item) ? trans('home.edit')  : trans('home.add')  ])
        ...
        <form action="{{ concatenateLangToUrl('admin/car/item') }}{{ isset($item) ? '/'.$item->id : '' }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
            @include("admin.car.relation.region.edit")
  ...
  <div class="form-group">
   <label for="youtube">{{ trans("car.youtube")}}</label>
    ...
    <br>
    @endif
    <input type="text" name="youtube" class="form-control" id="youtube" value="{{ isset($item->youtube) ? $item->youtube : old("youtube")  }}"  placeholder="{{ trans("car.youtube")}}">
   </label>
  </div>
              <div class="form-group">
